...
function HrPersonalRequestsList() {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "actions",
      render: (text, record) => (
        <Link to={`/hr/personal-requests/${record.requestId}`}>
          <Space size="middle">
            <ArrowRightOutlined />
          </Space>
        </Link>
      ...
      {loading ? (
        ...
        <p>
          <FormattedMessage
            ...
            defaultMessage={
              "We are currently experiencing an issue and can't provide the requests list. Please come back later."
            }
          />
        </p>
      )}
...
export default HrPersonalRequestsList;
